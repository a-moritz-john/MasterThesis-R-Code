ScoreSplit <- function(model = NULL, mydata = NULL, control = NULL,
  ...
  ############## CHANGE ##################
  # Scale the number of observations to be equal to the number of not missing data
  # n_obs <- nrow(Scores[Scores[, "covariance_model_S12"] != 0, , drop = FALSE])
  
  # Identify relevant parameters (has to be extended in the future)
  foc_idx <- match("covariance_model_S12", colnames(Scores))
  # Identify the rows to keep
  keep    <- abs(Scores[, foc_idx]) > 0
  # Subset the Scores
